...
import { bridgeConnection, streamIdForUserId, getUserParentStreamId, logErrorOnBridgeAccount } from "../lib/bridgeAccount.js";
import * as pryvService from "../lib/pryvService.js";
import { internalError, badRequest, serviceError } from "../errors/index.js";
import * as user from "./user.js";
import ShortUniqueId from 'short-unique-id';
import { advertiseNewUserToPlugins, requiredPermissionsAndStreams } from "../lib/plugins.js";
...
async function finalizeToBeCatched(partnerUserId, pollParam) {
    ...
    if (pollURL == null || !pollURL.startsWith('http'))
        badRequest('Missing or invalid prYvpoll URL');
...
async function authStatusStore(partnerUserId, payload) {
    ...
    if (!response[1].event || response[1].error)
        serviceError('Failed storing auth status', response[1]);
...
async function authStatusesClean(authStatusEvents) {
    if (!authStatusEvents || authStatusEvents.length < 1)
        return;
    const apiCalls = [];
    ...
    for (const r of res) {
        if (r.error)
            logger().error('Failed deleting status event', r);
        if (r.eventDeletion)
            logger().info(`Deleted status event id: ${r.eventDeletion.id}`);
    }
}
// ------- helpers
/**
...
function validatePermissions(permissions) {
    if (!Array.isArray(permissions))
        internalError('Permissions setting should be an array: ' + JSON.stringify(permissions, null, 2));
    if (permissions.length === 0)
        internalError('Permissions setting should have one element ' + JSON.stringify(permissions, null, 2));
    for (const p of permissions) {
        if (p.streamId === '*' && p.level === 'manage' && p.defaultName === undefined)
            continue;
        for (const k of ['streamId', 'level', 'defaultName']) {
            if (!p[k] || typeof p[k] !== 'string')
                internalError('Permissions setting is not valid ' + JSON.stringify(p, null, 2));
        }
    }
}
// --- webhook caller
