import { bridgeConnection, streamIdForUserId, getUserParentStreamId, getActiveUserStreamId } from "../lib/bridgeAccount.js";
import { unkownRessource, serviceError, badRequest } from "../errors/index.js";
...
async function addCredentialToBridgeAccount(partnerUserId, appApiEndpoint) {
    ...
    if (result[1]?.error?.id)
        throw serviceError('Failed add user credentials', result[1]);
    return result[1];
}
async function exists(partnerUserId) {
    ...
    const result = await bridgeConnection().api(apiCalls);
    if (result[0]?.error?.id === 'unknown-referenced-resource')
        return false;
    return true;
}
/**
...
async function status(partnerUserId, throwUnkown = true) {
    ...
    if (error)
        serviceError('Failed to get user status', error);
...
async function setStatus(partnerUserId, active) {
    ...
    if (resultUpdate[0]?.error?.id === 'unknown-referenced-resource')
        unkownRessource('Unkown user', { userId: partnerUserId });
    const errorUpdate = resultUpdate.error || resultUpdate[0]?.error;
    if (errorUpdate)
        serviceError('Failed to get user status', errorUpdate);
