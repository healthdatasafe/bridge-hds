...
import * as onboard from "../methods/onboard.js";
...
router.post('/onboard/', async (req, res) => {
    ...
    if (!redirectURLs)
        errors.badRequest('redirectURLs missing');
    ...
    for (const key of ['onboardingSecret', 'partnerUserId', 'status', 'error', 'errorObject', 'type', 'errorObjectJSON', 'pluginResultJSON', 'pluginResult']) {
        if (webhookClientData[key])
            errors.badRequest(`clientData.${key} is not  is a reserved key`);
    }
...
router.post('/:partnerUserId/status', async (req, res) => {
    ...
    if (active !== true && active !== false)
        errors.badRequest('active must be true or false');
